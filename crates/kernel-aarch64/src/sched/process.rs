...
pub fn intent_class_weight(class: IntentClass) -> f32 {
    match class {
        IntentClass::Unknown => 0.0,
        IntentClass::Compute => 1.0,
        IntentClass::RealTime => 0.95,
        IntentClass::IO => 0.7,
        IntentClass::System => 0.6,
        IntentClass::Background => 0.1,
    }
}
...
const _: () = {
    assert!(core::mem::size_of::<ProcessClass>() == 1);
    assert!(core::mem::align_of::<Process>() == 8);
};
...
impl Process {
    ...
    pub fn to_features(&self, current_tick: u64, max_pages: u32, max_caps: u16) -> [f32; 16] {
        ...
        let deadline_urgency = match self.deadline_tick {
            None => 0.0,
            Some(deadline_ns) if deadline_ns <= now_ns => 1.0,
            Some(deadline_ns) => {
                let remaining = deadline_ns.saturating_sub(now_ns);
                let window = deadline_ns.max(1);
                clamp01(1.0 - (remaining as f32 / window as f32))
            }
        };
        let intent_w = intent_class_weight(self.intent_class).max(intent_weight(self.stats.last_intent_id));
        let starvation_risk = ratio_u64(self.stats.ticks_waiting, tick_norm);
        let intent_recency = if self.stats.intent_fire_ns > 0 && now_ns > self.stats.intent_fire_ns {
            let age_ns = now_ns - self.stats.intent_fire_ns;
            let window_ns = 10_000_000u64;
            if age_ns < window_ns {
                1.0 - (age_ns as f32 / window_ns as f32)
            } else {
                0.0
            }
        } else {
